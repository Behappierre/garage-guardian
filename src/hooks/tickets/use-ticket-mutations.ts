  ...
  const submitTicketMutation = useMutation({
    mutationFn: async ({
      ...
      if (ticketId) {
        ...
        // For new tickets, don't include ticket_number - it's auto-generated by DB trigger
        // Use our custom function to handle job ticket creation
        const { data, error } = await supabase
          .rpc("create_job_ticket", {
            ...
            p_assigned_technician_id: dataWithGarage.assigned_technician_id,
            p_client_id: dataWithGarage.client_id,
            p_vehicle_id: dataWithGarage.vehicle_id,
            p_garage_id: dataWithGarage.garage_id
          });

        if (error) throw error;
        // The RPC function returns the job ticket ID directly as a string
